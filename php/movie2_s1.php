...
<html lang="en">
	...
	$res4=pg_query($db,"select * from m2s1;");
				$r4=pg_fetch_all($res4);
				//var_dump($r1);
				if($r4){
				if(count($r4)>0){
				foreach($r4 as $i){
				...
				echo "<input type=\"hidden\" name=\"filled4\" id=\"filled4\" value=$a4 />";
	...
	?><nav>
		...
		<img src="../img/movie2.jpg" alt="rocketry_movie" id="rocketry" />
			...
			<li><a href="movie2_s3.php"> 19:00 </a></li>
			</ul>
			
			<input type="hidden"  name="movie" value="m2s1" />
		...
		<header>
			...
			<label for="no_seats">No. of seats </label>
				...
				<input type="text" id="st" name="st" value="12:00" size=5 readonly />
